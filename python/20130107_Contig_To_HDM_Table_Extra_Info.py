...
## Usage: python ~/code/python/20130107_Contig_To_HDM_Table_Extra_Info.py <ALL BLAST RESULT> <SELECTED> <GENE SYMBOL> <OUTPUT>

import sys
if len(sys.argv) < 5:
        raise SystemExit, 'use grep "##" ~/code/python/20130107_Contig_To_HDM_Table_Extra_Info.py to get usage'
		...
		for list2 in best_90_percent_lists:
			if len(list2)-1 < i:
				...
				if prot_id in gs_dict:
					gs = gs_dict[prot_id]
				str1 = str1 + "\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (best_tokens[2], gs, best_tokens[3], best_tokens[8], best_tokens[9], best_tokens[10], best_tokens[11])
