...
## Usage: ~/code/bash/002b_Map_PE_Reads_To_Fasta.sh <INPUT FASTA> <OUTPUT PREFIX> <FASTQ 1> <FASTQ 2><NUM BASES TRIMMED AT 5'> <NUM BASES TRIMMED AT 3'> <INSERT SIZE MIN> <INSERT SIZE MAX> <MAPPED READS | - > <UNMAPPED READS | - >
set -o verbose
...
bowtie $bowtie_opt -t -I ${7} -X ${8} --fr -v 1 -p 20 "$index_name" -1 ${3} -2 ${4} ${2}_bowtie_out > ${2}_bowtie-stdout 2> ${2}_bowtie-stderr < /dev/null

/bin/rm /home/klui/install/bowtie-0.12.8/indexes/"$index_name"*

touch _BASH_002b_END
